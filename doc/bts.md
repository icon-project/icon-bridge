...
BTP Token Service is a smart contract which handles token transfer operations. It sends BTP message to other chains through BTP Message Center(BMC). Similarly, it recieves incoming requests from other chain via BMC, handles response and send a message back to source chain. 
...
1. Registers [BTS](bts.md) s for the services.
