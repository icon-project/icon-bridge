...
// #[derive(BorshDeserialize, BorshSerialize)]
// pub struct StorageBalances(HashMap<(AccountId, AssetId), StorageBalance>);
...
//     pub fn add(&mut self, account: AccountId, asset_id: AssetId) {
//         if !self.contains(&account, asset_id) {
//             self.0.insert((account, asset_id), u128::default());
//         }
//     }

//     pub fn remove(&mut self, account: AccountId, asset_id: AssetId) {
//         self.0.remove(&(account, asset_id));
//     }

//     pub fn get(&self, account: &AccountId, asset_id: &AssetId) -> Option<&StorageBalance> {
//         if let Some(storage_balance) = self.0.get(&(account.to_owned(), asset_id.to_owned())) {
...
//         if let Some(storage_balance) = self.0.get_mut(&(account.to_owned(), asset_id.to_owned())) {
//             return Some(storage_balance);
//         }
//         None
//     }
...
//     pub fn contains(&self, account: &AccountId, asset_id: AssetId) -> bool {
//         return self.0.contains_key(&(account.to_owned(), asset_id));
//     }
