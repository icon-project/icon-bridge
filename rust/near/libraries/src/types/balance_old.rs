...
pub struct OldAccountBalance {
    deposit: Balance,
    refundable: Balance,
    locked: Balance,
}

impl OldAccountBalance {
...
pub struct OldBalances(LookupMap<(AccountId, OldAssetId), OldAccountBalance>);
...
impl OldBalances {
    ...
    pub fn add(&mut self, account: &AccountId, asset_id: &OldAssetId) {
        if !self.contains(account, asset_id) {
            self.0.insert(
                &(account.to_owned(), asset_id.to_owned()),
                &OldAccountBalance::default(),
            );
        }
    }

    pub fn remove(&mut self, account: &AccountId, asset_id: &OldAssetId) {
        self.0.remove(&(account.to_owned(), asset_id.to_owned()));
    }

    pub fn get(&self, account: &AccountId, asset_id: &OldAssetId) -> Option<OldAccountBalance> {
    ...
    pub fn contains(&self, account: &AccountId, asset_id: &OldAssetId) -> bool {
        return self
            .0
    ...
    pub fn set(
        ...
        account: &AccountId,
        asset_id: &OldAssetId,
        account_balance: OldAccountBalance,
    ) {
        self.0
...
mod tests {
    ...
    fn add_balance() {
        ...
        testing_env!(context);
        let mut balances = OldBalances::new();
