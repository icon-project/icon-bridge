...
impl MerkleTreeAccumulator {
    ...
    pub fn init_from_serialized<H: Hasher>(&mut self, rlp_bytes: &[u8]) {
        ...
        if unpacked.len() > 1 {
            ...
            for root in &serialized_roots {
                let hash =
                    Hash::new::<H>(&RlpItem::try_to_vec(root).expect("Failed to convert to vec"));
    ...
    pub fn set_cache(&mut self, hash: &Hash) {
        ...
        if self.cache.len() > self.cache_size {
            ...
            for (i, _) in self.cache.iter().enumerate() {
    ...
    pub fn verify<H: Hasher>(
        ...
        if self.height == at {
            ...
            if !self.newer_witness_allowed {
                return Err(MtaError::InvalidWitnessNewer {
                    message: "newer witness not allowed",
                });
            }
            if self.height < height {
                return Err(MtaError::InvalidWitnessNewer {
                    message: "given witness for newer node",
                });
            }
            root_idx = self
        ...
        {
            return Err(MtaError::InvalidWitnessOld {
