...
class BTPPreiphery(sp.Contract):
    ...
    def add_to_blacklist(self, params):
        ...
        sp.for i in sp.range(sp.nat(0), sp.len(params)):
            parsed_addr = parse_addr.str_to_addr(params.get(i))
    ...
    def handle_btp_message(self, _from, svc, sn, msg):
        ...
        with sm.serviceType.match_cases() as arg:
            ...
            with arg.match("BLACKLIST_MESSAGE") as a2:
                bm = rlp_decode.decode_blacklist_msg(sm.data)
                addresses = bm.addrs
            ...
            with arg.match("CHANGE_TOKEN_LIMIT") as a3:
                tl = rlp_decode.decode_token_limit_msg(sm.data)
            ...
            with arg.match("RESPONSE_HANDLE_SERVICE") as a4:
                ...
                response = rlp_decode.decode_response(sm.data)
    ...
    def handle_btp_error(self, svc, sn, code, msg):
        ...
        sp.verify(svc == self.service_name, "InvalidSvc")
        sp.verify(sp.len(sp.pack(self.data.requests[sn].from_)) != 0, "InvalidSN")
    ...
    def handle_response_service(self, sn, code, msg):
        ...
        caller = sp.local("caller", parse_addr.str_to_addr(self.data.requests[sn].from_), sp.TAddress).value
...
def test():
    ...
    bts_core = sp.test_account("Admin")

    scenario = sp.test_scenario()
    counter = BTPPreiphery(bmc.address, bts_core.address)
    scenario += counter
