...
class DecodeEncodeLibrary:
    ...
    def decode_response(self, rlp):
        ...
        with sp.if_(is_error.value == "Success"):
            ...
            msg.value = sp.view("decode_string", self.data.helper, temp_byt.value, t=sp.TString).open_some()
        return sp.record(rv = sp.record(code=temp_int.value, message = msg.value),status = is_error.value)

    def decode_service_message(self, rlp):
        sp.set_type(rlp, sp.TBytes)

        rlp_sm = sp.local("rlp_sm_bts", sp.map(tkey=sp.TNat))
        is_error = sp.local("error_in_bts_decode_service_message", sp.string("Success"))
        is_list_lambda = sp.view("is_list", self.data.helper, rlp, t=sp.TBool).open_some()
        with sp.if_(is_list_lambda):
        ...
        return sp.record(rv=sp.record(serviceType=_service_type.value,
                         data=temp_byt.value), status=is_error.value)
    ...
    def decode_transfer_coin_msg(self, rlp):
        ...
        rv1 = sp.local("rv1_bts_transfer_coin", sp.string(""))
        rv2 = sp.local("rv2_bts_transfer_coin", sp.string(""))
        ...
        with sp.if_(is_error.value == "Success"):
            ...
            with sp.if_(is_error.value == "Success"):
                ...
                sp.for x in new_sub_list.items():
                    ...
                    rv1.value = sp.view("decode_string", self.data.helper, rv1_byt.value, t=sp.TString).open_some()
                    rv2.value = sp.view("decode_string", self.data.helper, rv2_byt.value, t=sp.TString).open_some()
        return sp.record(value=sp.record(from_addr= rv1.value, to = rv2.value, assets = rv_assets.value), status = is_error.value)
