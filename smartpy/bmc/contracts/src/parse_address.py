...
test_str = ""
test_addr = sp.address("KT1GPaeWzJpZAoY45v4aEWBFiqp6vtkLUkgw")


def unforge_address(data):
    ...
    sp.for item in tz_prefixes.items():
        sp.if item.key == prefix:
            return base58_encode(sp.slice(byt, 2, 20).open_some(), Utils.Bytes.of_string(item.value), local_byte)

    sp.if (starts_with == sp.bytes("0x01")) &  (ends_with == sp.bytes("0x00")):
        return base58_encode(sliced_byte, Utils.Bytes.of_string("KT1"), local_byte)
    sp.if (starts_with == sp.bytes("0x02")) & (ends_with == sp.bytes("0x00")):
        return base58_encode(sliced_byte, Utils.Bytes.of_string("txr1"), local_byte)
    sp.if (starts_with == sp.bytes("0x03")) & (ends_with == sp.bytes("0x00")):
        return base58_encode(sliced_byte, Utils.Bytes.of_string("sr1"), local_byte)
...
def base58_encode(v, prefix, _byte):
    """
    ...
    sp.for enc in base58_encodings:
        sp.if (length_v == Utils.Int.of_string(enc["len"])) & (prefix == Utils.Bytes.of_string(enc["prefix"])) :
            encoding.value = enc
            byte_from_tbl.value = tb([sp.as_nat(Utils.Int.of_string(enc["elem1"])),
...
def add_to_str(params):
    sp.set_type(params, sp.TAddress)
    return unforge_address(sp.pack(params))

def str_to_addr(params):
    ...
    sp.if sp.len(string_in_bytes) == sp.nat(42):
        ...
        sp.for enc in base58_encodings:
            byte_local.value = tb([sp.as_nat(Utils.Int.of_string(enc["elem1"])),
                                              sp.as_nat(Utils.Int.of_string(enc["elem2"])),
                                                sp.as_nat(Utils.Int.of_string(enc["elem3"]))])
        ...
        sp.if actual_prefix.value == "KT1":
            decoded_address = sp.unpack(sp.bytes("0x050a00000016") + sp.bytes("0x01") + v.open_some() + sp.bytes("0x00"),
                            sp.TAddress)
            return decoded_address.open_some()
        sp.if actual_prefix.value == "txr1":
            decoded_address = sp.unpack(sp.bytes("0x050a00000016") + sp.bytes("0x02") + v.open_some() + sp.bytes("0x00"),
                            sp.TAddress)
            return decoded_address.open_some()
        sp.if actual_prefix.value == "sr1":
            decoded_address = sp.unpack(sp.bytes("0x050a00000016") + sp.bytes("0x03") + v.open_some() + sp.bytes("0x00"),
                            sp.TAddress)
            return decoded_address.open_some()

        sp.if actual_prefix.value == "":
            return sp.address("tz1ZZZZZZZZZZZZZZZZZZZZZZZZZZZZNkiRg")
...
#
#
# sp.add_compilation_target("smartpy-utils", Conversion())
