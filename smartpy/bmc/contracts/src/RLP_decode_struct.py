...
def decode_bmc_message(rlp):
    ...
    sp.for k in rlp_.items():
        sp.if counter.value == 0:
            temp_map_string["src"] = decode_string(k.value)
        sp.if counter.value == 1:
            temp_map_string["dst"] = decode_string(k.value)
        sp.if counter.value == 2:
            temp_map_string["svc"] = decode_string(k.value)
...
def decode_propagate_message(rlp):
    decode_list = sp.build_lambda(Utils.RLP.Decoder.decode_list)
    decode_string = sp.build_lambda(Utils.RLP.Decoder.decode_string)
    rlp_ = decode_list(rlp)
    ...
    sp.for d in rlp_.items():
        sp.if counter.value == 0:
            temp_string.value = decode_string(d.value)
        counter.value = counter.value + 1
    return temp_string.value

def decode_init_message(rlp):
    decode_list = sp.build_lambda(Utils.RLP.Decoder.decode_list)
    decode_string = sp.build_lambda(Utils.RLP.Decoder.decode_string)
    rlp_ = decode_list(rlp)
    ...
    sp.for x in new_sub_list.items():
        _links.value.push(decode_string(x.value))
        counter.value = counter.value + 1
    return _links.value
...
def decode_bmc_service(rlp):
    ...
    sp.for b in rlp_.items():
        sp.if counter.value == 0:
            temp_string.value = decode_string(b.value)
        sp.if counter.value == 1:
...
def decode_gather_fee_message(rlp):
    ...
    sp.for c in rlp_.items():
        ...
        sp.if counter.value == 0:
            temp_str.value = decode_string(c.value)
        counter.value = counter.value + 1
...
def to_message_event(rlp):
    ...
    sp.for i in rlp_.items():
        sp.if counter.value == 2:
            rv3.value = i.value
        sp.if counter.value == 0:
            rv1.value = decode_string(i.value)
        sp.if counter.value == 1:
...
def decode_receipt_proof(rlp):
    ...
    sp.for z in new_sub_list.items():
        ...
        sp.for a in decode_list(sub_list.value).items():
            events.value[counter.value] = to_message_event(a.value)
            counter.value = counter.value + 1
    return sp.record(index = rv_int.value, events = events.value, height = rv_int2.value)


def decode_receipt_proofs(rlp):
    decode_list = sp.build_lambda(Utils.RLP.Decoder.decode_list)
    rlp_ = decode_list(rlp)
    ...
    sp.if sp.len(new_sub_list) > 0:
        sp.for x in rlp_.items():
            receipt_proofs.value[counter.value] = decode_receipt_proof(x.value)
            counter.value = counter.value + 1
