...
# update localnet sharding hostnames to deployed host address
ARG SHARDING_HOST="127.0.0.1"
RUN sed -i "s/127.0.0.1:%d/${SHARDING_HOST}:%d/" internal/configs/sharding/localnet.go

RUN scripts/install_build_tools.sh

RUN go mod tidy
RUN rm -rf tmp_log* *.rlp .dht*
...
CMD ["./test/deploy.sh", "-e", "-B", "-D", "600000", "./test/configs/local-resharding.txt"]
