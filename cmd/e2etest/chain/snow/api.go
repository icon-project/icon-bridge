...
func NewApi(l log.Logger, cfg *chain.Config) (chain.ChainAPI, error) {
	...
	if err != nil {
		l.Errorf("failed to create snow rpc client: url=%v, %v", cfg.URL, err)
		return nil, err
	}
	r := &api{
		log:      l,
...
func (a *api) GetCoinBalance(coinName string, addr string) (*chain.CoinBalance, error) {
	...
	if !strings.Contains(addr, ".snow") {
		return nil, fmt.Errorf("Address should be BTP address of account in native chain snow. Got %v", addr)
	}
...
func (a *api) Transfer(coinName, senderKey, recepientAddress string, amount *big.Int) (txnHash string, err error) {
	...
	within := false
	if strings.Contains(recepientAddress, ".snow") {
		within = true
...
func (a *api) GetBTPAddress(addr string) string {
	fullAddr := "btp://" + a.requester.networkID + ".snow/" + addr
	return fullAddr
}
