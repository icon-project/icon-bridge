...
import (
	...
	btscore "github.com/icon-project/icon-bridge/bmr/cmd/e2etest/chain/hmny/abi/btscore"
	erc20 "github.com/icon-project/icon-bridge/bmr/cmd/e2etest/chain/hmny/abi/erc20tradable"
	hrc20 "github.com/icon-project/icon-bridge/bmr/cmd/e2etest/chain/hmny/abi/hrc20"
...
func newRequestAPI(url string, l log.Logger, contractNameToAddress map[chain.ContractName]string, networkID string) (*requestAPI, error) {
	...
	caddr, ok = contractNameToAddress[chain.TONEHmny]
	if !ok {
		return nil, fmt.Errorf("contractNameToAddress doesn't include %v", chain.TONEHmny)
	}
