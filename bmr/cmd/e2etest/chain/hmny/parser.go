...
import (
	...
	btsp "github.com/icon-project/icon-bridge/bmr/cmd/e2etest/chain/hmny/abi/btsperiphery"
...
func NewParser(url string, nameToAddr map[chain.ContractName]string) (*parser, error) {
	...
	btsperiAddr, ok := nameToAddr[chain.BTSPeripheryHmny]
	if !ok {
		return nil, fmt.Errorf("nameToAddr doesn't include %v ", chain.BTSPeripheryHmny)
	}
