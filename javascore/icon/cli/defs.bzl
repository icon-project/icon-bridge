def create_keystore(name):
    ...
    native.genrule(
        ...
        cmd = """$(execpath @com_github_icon_project_goloop//cmd/goloop:goloop) rpc --uri $$(cat $(location @icon//:node_url)) sendtx transfer --to $$(cat $(location @icon//cli:get_wallet_%s_keystore)) --value 0xad78ebc5ac6200000 --key_store $$(cat $(location @icon//:goloop_config_dir))/keystore.json --key_secret $$(cat $(location @icon//:goloop_config_dir))/keysecret --nid "$$(cat $(location @icon//:network_id))" --step_limit 5000000000 >$@""" % name,
        executable = True,
        local = True,
    )
    native.genrule(
...
def configure_bmr(name):
    native.genrule(
        ...
        srcs = [
            "@icon//cli:create_keystore_%s" % name,
        ],
        cmd = "echo \"$$(cat $(location @icon//cli:create_keystore_%s))\" | jq -r .  > $@" % name,
        executable = True,
        local = True,
        tools = [
            "@icon//cli:transfer_amount_%s_address" % name,
