...
provision() {
  ...
  export PRIVATE_KEY="[\""$(cat $ICONBRIDGE_CONFIG_DIR/env)"\"]"
  ...
  cp $ICON_KEY_STORE $CONFIG_DIR/keystore/icon.bts.wallet.json
  cp $ICON_KEY_STORE $CONFIG_DIR/keystore/icon.bmc.wallet.json
  cp $ICON_KEY_STORE $CONFIG_DIR/keystore/icon.bmr.wallet.json

  cp $BSC_KEY_STORE $CONFIG_DIR/keystore/bsc.bts.wallet.json 
  cp $BSC_KEY_STORE $CONFIG_DIR/keystore/bsc.bmc.wallet.json
  cp $BSC_KEY_STORE $CONFIG_DIR/keystore/bsc.bmr.wallet.json
  ...
  jq -n '
    ...
    --argjson b2i_relay "$(
      jq -n '
        ...
        --argfile dst_key_store "$ICON_KEY_STORE" \
        --arg dst_key_store_cointype "icx" \
        --arg dst_key_password "$(cat $ICON_SECRET)" \
